...
Final Project
ai_pong.py
This program has two AI Paddles play against each other.
"""
...
testing_input = []
testing_input_player = []
have_new_input = False
have_new_input_player = False
global_testing_output = []
global_testing_input = []
left_just_scored = False


class Player():
        def __init__(self, backprop):
                self.x, self.y = 16, convert_correct_form_to_location(6, size_of_spot)
                self.goal_y = self.y
                self.speed = 3
        ...
        def movement(self):
                global have_new_input_player
                global testing_input_player
                # We will only move this paddle if new input has been given
                if have_new_input_player:
                        # Our inputs are way too big, so we scale them down
                        scaled_testing_input = [x/100 for x in testing_input_player]
...
class Ball():
        ...
        def movement(self):
                global global_testing_input
                global global_testing_output
                global left_just_scored
                self.x += self.speed_x
                ...
                if self.x <= 0:
                        self.__init__()
                        AiPaddle.score += 1
                        # We move the player paddle back to a spot so it can hit the ball
                        player.y = convert_correct_form_to_location(6, size_of_spot)
                        player.goal_y = player.y
                        ...
                        # Need to get rid of last input to left paddle
                        global_testing_input = global_testing_input[:-1]
                        left_just_scored = True
                ##wall col
                #paddle col
                #player
                for n in range(-self.size, player.padHei):
                        if self.y == player.y + n:
                                if self.x <= player.x + player.padWid:
                                        ...
                                        global_testing_input.append(testing_input)
                                        break
                        n += 1
                #AiPaddle
                for n in range(-self.size, AiPaddle.padHei):
                        if self.y == AiPaddle.y + n:
                                if self.x >= AiPaddle.x - AiPaddle.padWid:
                                        ...
                                        global have_new_input_player
                                        global testing_input_player
                                        have_new_input_player = True
                                        testing_input_player = [AiPaddle.y] + self.y_prev_list + [self.y]
                                        if not left_just_scored:
                                                global_testing_output.append(AiPaddle.y)
                                        else:
                                                left_just_scored = False
                                        break
                        n += 1
...
def save_data(fileName):
        ...
        with open(fileName, 'wb') as f:
                pickle.dump((global_testing_input, global_testing_output), f)
...
bprop.save("./pickled_data/trained_network_2.dat")
"""
...
# trained_network_2.dat has no training data for when the ball gets by the computer
        # We get into a cycle of losing after one ball misses
...
while True:
        ...
        for event in pygame.event.get():
                        ...
                        if event.type == pygame.QUIT:
                                ...
                                save_data("./pickled_data/testing_set.dat")
                                exit()
        ##process
        #logic
        ball.movement()
